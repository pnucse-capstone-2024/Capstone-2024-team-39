...
def load_total_energy(env):
    ...
    if env.load_from_replay_path is not None:
        total_energy_charged = 0
        total_energy_discharged = 0
        return total_energy_charged, total_energy_discharged
        ...
        for i, tr in enumerate(env.charging_network_topology):
            for cs in env.charging_network_topology[tr]['charging_stations']:
                ev_charger = EV_Charger(id=cs_counter,
                                        ...
                                        verbose=env.verbose,)
                cs_counter += 1
                total_energy_charged = ev_charger.get_total_energy_charged
                total_energy_discharged = ev_charger.get_total_energy_discharged
...
################# add average_price ######################
def load_electricity_prices(env) -> Tuple[np.ndarray, np.ndarray, float]:
    ...
    if env.load_from_replay_path is not None:
        return env.replay.charge_prices, env.replay.discharge_prices, env.replay.average_price
    ...
    for i in range(env.simulation_length):
        ...
        try:
            price = data.loc[(data['year'] == year) & (data['month'] == month) & 
                             (data['day'] == day) & (data['hour'] == hour), 
                             'Price (EUR/MWhe)'].iloc[0] / 1000  # €/kWh

            charge_prices[:, i] = -price  # Charge price (negative for cost)
            discharge_prices[:, i] = price  # Discharge price

            # Aggregate price for daily average calculation
            average_price = np.mean(charge_prices)
        except:
    ...
    average_price = np.mean(average_price)

    return charge_prices, discharge_prices, average_price
