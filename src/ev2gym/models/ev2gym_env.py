...
##
from ev2gym.utilities.loaders import load_total_energy
##
...
class EV2Gym(gym.Env):
    ...
    def __init__(self,
        ...
        ##### Load total energy charged / discharged #####
        self.total_energy_charged, self.total_energy_discharged = load_total_energy(self)
        ##################################################

        # Load Electricity prices for every charging station
        ###### Load average price ########################
        self.charge_prices, self.discharge_prices, self.average_price = load_electricity_prices(
            self)
    ...
    def _get_observation(self):
        ...
        '''
        # Ensure each part of the state has a consistent shape and check the state shape
        current_step = [self.current_step]  # Wrap in list to ensure it has consistent length
        current_power_usage = [self.current_power_usage[self.current_step - 1]]  # Wrap in list
        charge_prices = abs(self.charge_prices[0, self.current_step:self.current_step + 20])
        if len(charge_prices) < 20:
            charge_prices = np.append(charge_prices, np.zeros(20 - len(charge_prices)))  # Ensure length 20

        # Ensure each additional state component is properly shaped
        total_energy_charged = [self.total_energy_charged] if np.isscalar(self.total_energy_charged) else self.total_energy_charged
        battery_state = [self.battery_state] if np.isscalar(self.battery_state) else self.battery_state
        average_user_satisfaction = [self.average_user_satisfaction] if np.isscalar(self.average_user_satisfaction) else self.average_user_satisfaction

        # Combine all elements into the state list, ensuring consistent shapes
        state = [
            current_step,
            current_power_usage,
            charge_prices,
        ...
        # Flatten state list and convert to numpy array
        state_array = np.array(np.hstack(state))
        return state_array
        '''
        # Ensure each part of the state has a consistent shape and check the state shape
        current_step = np.array([self.current_step])  # Current step as an array
        current_power_usage = np.array([self.current_power_usage[self.current_step - 1]])  # Current power usage as an array
            
        # Get charge prices and ensure length is 20
        charge_prices = np.abs(self.charge_prices[0, self.current_step:self.current_step + 20])
        if len(charge_prices) < 20:
            charge_prices = np.append(charge_prices, np.zeros(20 - len(charge_prices)))  # Ensure length 20
            
        # Convert other state variables to arrays
        total_energy_charged = np.array([self.total_energy_charged])
        battery_state = np.array(self.battery_state)  # Assuming battery_state is already a numpy array
        average_user_satisfaction = np.array([self.average_user_satisfaction])


        if np.isnan(current_power_usage).any():
            print("NaN in current_power_usage:", current_power_usage)
        if np.isnan(charge_prices).any():
            print("NaN in charge_prices:", charge_prices)
        if np.isnan(total_energy_charged).any():
            print("NaN in total_energy_charged:", total_energy_charged)
        if np.isnan(battery_state).any():
            print("NaN in battery_state:", battery_state)
        if np.isnan(average_user_satisfaction).any():
            print("NaN in average_user_satisfaction:", average_user_satisfaction)
        # Combine all elements into a single state array, ensuring consistent shapes
        state = np.hstack([current_step,
                            current_power_usage,
                            charge_prices,
                            total_energy_charged,
                            battery_state,
    ...
    #########################################
    def charge_EV(self, ev, energy_to_charge):
        # Implement logic to charge EV and update total_energy_charged
        self.total_energy_charged += energy_to_charge
        # Additional logic for updating battery_state and user satisfaction
