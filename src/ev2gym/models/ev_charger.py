...
'''
    ...
    ########### average_price addition ################
    def step(self, actions, charge_price, discharge_price, average_price):
        '''
        Updates the EV charger status according to the actions taken by the EVs
        Inputs:
            - actions: a list of actions taken by the EVs connected to the EV charger
            - charge_price: the price of charging per kWh in the current timestep
            - discharge_price: the price of discharging per kWh in the current timestep
            - average_price: the average price for the day
        ...
        self.current_charge_price = charge_price
        self.current_discharge_price = discharge_price
        self.current_average_price = average_price
        self.current_signal = []
        ...
        for i, action in enumerate(normalized_actions):
            ...
            amps = 0
            # Determine whether to charge or discharge based on prices
            if action > 0 and charge_price < average_price:  # Charging condition
                amps = action * self.max_charge_current
                if amps < self.min_charge_current - 0.01:
                    amps = 0
            ...
            elif action < 0 and charge_price >= average_price:  # Discharging condition
                amps = action * abs(self.max_discharge_current)
                if amps > self.min_discharge_current - 0.01:
                    amps = self.min_discharge_current
                ...
                # Print discharging information
                # print(f'Discharging EV {self.evs_connected[i].id}: '
                #     f'Action: {action}, Amps: {amps}, Energy: {actual_energy}, '
                #     f'Discharge Price: {discharge_price}, Total Amps: {self.current_total_amps}')

            # If action is zero and EV is connected
            if action == 0 and self.evs_connected[i] is not None:
                actual_energy, actual_amps = self.evs_connected[i].step(amps, self.voltage)
        ...
        for i, ev in enumerate(self.evs_connected):
            if ev is not None:
                if ev.is_departing(self.current_step) is not None:
                    ...
                    departing_evs.append(ev)
                    if self.verbose:
                        print(f'- EV {ev.id} is departing from CS {self.id} port {i} with user satisfaction {ev_user_satisfaction} (SoC: {ev.get_soc() * 100:6.1f}%)')
    ...
    def get_total_energy_charged(self):
        return self.total_energy_charged
    
    def get_total_energy_discharged(self):
        return self.total_energy_discharged
